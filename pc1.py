...
def get_word_counts_download_link(word_counts, filename, text_btn="下载"):
    """生成一个下载词频统计的链接"""
...
def main():
    ...
    if text is not None:
        ...
        if st.button('保存词频统计'):  # 添加一个按钮
            word_counts = get_top_words(text, num_words)
            download_link = get_word_counts_download_link(word_counts, 'word_counts.txt')
            st.markdown(download_link, unsafe_allow_html=True)
            st.success('点击上方链接下载词频统计')
